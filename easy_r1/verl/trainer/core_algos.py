...
def compute_grpo_outcome_advantage(
    ...
    # Collapse token rewards to one score per sample.
    ...
    bsz = scores.shape[0]
    for i in range(bsz):
        # Collect all rollouts for the same prompt.
        id2score[index[i]].append(scores[i])

    # Compute per-group statistics.
    for idx in id2score:
        # GRPO requires at least two samples per prompt.
    ...
    # Broadcast the normalized score back to token level.
    returns = scores.unsqueeze(-1) * response_mask  # (batch_size, response_length)

    # GRPO uses identical advantages and returns.
