...
class FSDPWorker(Worker):
    ...
    def __init__(
        ...
        # NCCL
        # NCCL NVIDIA Collective Communications Library NVIDIA GPU
        if not dist.is_initialized():
            dist.init_process_group(backend="nccl")

        # TF32 BF16
        # TF32: TensorFloat-32 NVIDIA Ampere GPU
        ...
        # Parameter Offload CPU GPU
        # GPU
        self._use_param_offload = False # Actor/Critic
    ...
    def _init_dist_mesh(self, config: Union[ActorConfig, CriticConfig], role: Literal["actor", "critic"]):
        """Init Dist Mesh."""
        world_size = dist.get_world_size() # GPU
        
        # ========== FSDP ==========
        fsdp_size = config.fsdp.fsdp_size
        if fsdp_size <= 0 or fsdp_size >= world_size:
            # FSDP GPU FSDP
            # 8 GPU → mesh_shape=(8,) GPU 1/8
            self.device_mesh = init_device_mesh("cuda", mesh_shape=(world_size,), mesh_dim_names=("fsdp",))
        else:
            # HSDP Hybrid Sharded Data Parallel DDP FSDP
            # 8 GPU fsdp_size=4 → mesh_shape=(2, 4)
            # - 2 DDP
            # - 4 GPU FSDP
        ...
        if config.ulysses_size > 1:
            # Ulysses GPU
            # 8 GPU ulysses_size=4 → mesh_shape=(2, 4)
            # - 2
            # - 4 GPU GPU 1/4
            self.ulysses_device_mesh = init_device_mesh(
                "cuda",
                mesh_shape=(world_size // config.ulysses_size, config.ulysses_size),
                mesh_dim_names=("dp", "sp"),  # dp: data parallel, sp: sequence parallel
            )
        else:
            # Ulysses
    ...
    def _build_model_optimizer(
        ...
        if role != "ref":  # ref model's tokenizer is same as actor
            ...
            self.model_config = AutoConfig.from_pretrained(
                ...
                trust_remote_code=model_config.trust_remote_code,
                bos_token_id=self.tokenizer.bos_token_id, # token ID
                eos_token_id=self.tokenizer.eos_token_id, # token ID
                pad_token_id=self.tokenizer.pad_token_id, # token ID
    ...
    def init_model(self):
        """Init Model."""
        # ========== Critic ==========
        # Critic GAE Generalized Advantage Estimation
        # GRPO RLOO Critic
        if self._has_critic:
        ...
        if self._has_ref:
            self._build_model_optimizer(
                model_config=self.config.actor.model, # Actor
        ...
        # ========== Actor ==========
        # DataParallelPPOActor Actor
        if self._has_actor:
            from .actor.dp_actor import DataParallelPPOActor  # lazy import

            self.actor = DataParallelPPOActor(
                config=self.config.actor,
                actor_module=self.fsdp_module, # FSDP
                actor_optimizer=self.optimizer,
            )

        # ========== Critic ==========
        # DataParallelPPOCritic Critic
        if self._has_critic:
        ...
        if self._has_ref:
            ...
            self.ref_policy = DataParallelPPOActor(
                ...
                actor_module=self.ref_fsdp_module, # FSDP
                # optimizer
            )
        ...
        if self._has_actor or self._has_critic:
            ...
            self.checkpoint_manager = FSDPCheckpointManager(
                ...
                processing_class=self.processor or self.tokenizer, # tokenizer
            )
...
#================================================================================
        ...
        if "uid" in self._cache:
            ...
            if len(current_uid) != len(cached_uid) or not np.array_equal(current_uid, cached_uid):
                self._cache.clear()
#================================================================================
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            global_num_tokens = data.meta_info["global_token_num"] # token
            
            # FLOPS Floating Point Operations
            estimated_flops, promised_flops = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
            
            # MFU Model FLOPs Utilization
            # MFU = ( FLOPS * PPO epochs) / ( FLOPS * GPU)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_answer_sequences(self, question_batch: DataProto):
        ...
        question_batch.meta_info.update(meta_info)

        question_batch = self.rollout_sharding_manager.preprocess_data(question_batch)
        
        # prompt
        output = self.rollout.generate_answer_sequences(question_batch=question_batch)
